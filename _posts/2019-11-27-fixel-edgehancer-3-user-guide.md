...
This user guide presents [Fixel EdgeHancer 3][98] to the user with simple guidelines on how to use properly to achieve optimal results.
 ...
 *	Run the installer inside the `ZIP` file:
	...
	*	macOS - *Double Click* on `Fixel EdgeHancer - macOS Installer.dmg` disk image and follow instructions.  
		The user may be asked for *Administrator* privileges in order to install the Plug In.
 *	Start *Adobe Photoshop* and launch the UI by going `Window -> Extensions -> Fixel EdgeHancer 3` (See below).

If any issue arises, please refer to [Fixel EdgeHancer 3 Installation Guide][03].
...
In the above figure one could see 6 images where one is the *Original Image* and all the other are created by applying [Fixel EdgeHancer 3][98] with different *radius* as written.
